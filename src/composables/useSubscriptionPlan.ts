...
import { PLAN_LIMITS, getLimits, getPlanName } from '@/utils/planLimits'
...
export function useSubscriptionPlan(plan: ClinicPlan) {
  ...
  const canAccessEmailNotifications = computed(() => limits.value.emailNotificationsEnabled)
  const canAccessCustomBranding = computed(() => limits.value.customBrandingEnabled)
  const canAccessFullAnalytics = computed(() => limits.value.fullAnalyticsEnabled)
